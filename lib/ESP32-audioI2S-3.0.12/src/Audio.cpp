...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// clang-format off
Audio::Audio(const char *root_ca, bool internalDAC /* = false */, uint8_t channelEnabled /* = I2S_SLOT_MODE_STEREO */, uint8_t i2sPort) {
...
bool Audio::connecttorequest(const char *host, String &request_str, uint8_t *payload_buf, size_t payload_len, bool ssl, uint8_t expected_codec) {
  xSemaphoreTake(mutex_playAudioData, portMAX_DELAY);
  ...
  if(res) {
      _client->write((const uint8_t *) request_str.c_str(), request_str.length());
      if(payload_buf && payload_len > 0) {
        size_t sent_bytes = 0;
        while(sent_bytes < payload_len){
            size_t sent = _client->write(&payload_buf[sent_bytes], payload_len - sent_bytes);
            if (sent == 0){
                log_w("Failed to send chunk! Lets wait a bit");
                delay(100);
                sent = _client->write(&payload_buf[sent_bytes], payload_len - sent_bytes);
                if (sent == 0){
      ...
      m_expectedCodec = expected_codec;
...
bool Audio::parseHttpResponseHeader() { // this is the response to a GET / request
    ...
    uint32_t ctime = millis();
    uint32_t timeout = m_f_ssl ? m_timeout_ms_ssl : m_timeout_ms; // ms
